      ...
      <div className={styles.container}>
          ...
          <div className={styles.header}>
            <Link href="links/createInstruction" legacyBehavior>
              <button className={styles.newInstruction__button}>
                <BsPlus size={20} />
                {authUser ? <> New Instruction </> : <> Create Instruction </>}
              </button>
            </Link>

            <button
              className={`${styles.closeMenu__button} ${
                !isOpen ? styles.openMenu__button : ""
              }`}
              onClick={handleMenu}
            >
              <TbLayoutSidebar size={17} />
            </button>
          </div>
        ...
        <div className={styles.footer}>
          {authUser ? (
            <CustomButton
              label="Log out"
              gradient={linearGradients.redLinearGradient}
            ...
            <div style={{ display: "flex", flexDirection: "row", gap: "1rem" }}>
              ...
              <CustomButton
                label="sign up"
                gradient={linearGradients.blueLinearGradient}
                path="/auth/signup"
              />
            </div>
          )}
